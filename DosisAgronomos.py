...
FechaActual = st.date_input(f"Fecha")

lote = st.selectbox(f"Lote", options=list(lotesConHectareas[camp1].keys()), key=f"Lote_1")
...
Dosis_por_hectarea = Cantidad/hectateras_lote
Cantidadhc = st.text_input(f"Dosis Por Hectárea de {Insumo}", value=str(Dosis_por_hectarea), disabled= True)
...
if agregar_este_insumo == "":
    data = {
        ...
        "Campo": [camp1],
        "Lote": [lote],
        "Hectáreas del Lote": [hectateras_lote],
